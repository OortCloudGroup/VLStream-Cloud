...
//	struct rtsp_demo *d = cc->demo;
	...
	if (s->vcodec_id != RTSP_CODEC_ID_NONE && rtsp_path_match(vpath, reqmsg->hdrs.startline.reqline.uri.abspath)) {
		...
		isaudio = 1;
	} else if (rtsp_path_match(s->path, reqmsg->hdrs.startline.reqline.uri.abspath)) {
		if (s->vcodec_id != RTSP_CODEC_ID_NONE && s->acodec_id == RTSP_CODEC_ID_NONE) {
			isaudio = 0;
			info("rtsp aggregate SETUP matched single video track: %s\n", reqmsg->hdrs.startline.reqline.uri.abspath);
		} else if (s->acodec_id != RTSP_CODEC_ID_NONE && s->vcodec_id == RTSP_CODEC_ID_NONE) {
			isaudio = 1;
			info("rtsp aggregate SETUP matched single audio track: %s\n", reqmsg->hdrs.startline.reqline.uri.abspath);
		} else {
			warn("rtsp aggregate SETUP ambiguous for path:%s\n", reqmsg->hdrs.startline.reqline.uri.abspath);
			rtsp_msg_set_response(resmsg, 461);
			return 0;
		}
	} else {
