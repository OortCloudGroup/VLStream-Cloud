<template>
  <div id="map-container" @click.capture="warnClick">
    ...
    >
      ...
      <MarkerList ref="markerListRef" :markers="markers" :popup-type="popupType" />
      <slot />
    </LMap>
  </div>
</template>
...
const emits = defineEmits(['warnClick'])
...
const initTrackPlayer = () => {
  ...
  if (pathCoordinates.value.length > 0) {
    ...
    L.marker(startCoordinates, { icon: startPointIcon }).addTo(leafLetMap.value)
    L.marker(endCoordinates, { icon: endPointIcon }).addTo(leafLetMap.value)
  }
  // 只在首次初始化时调整地图视图
