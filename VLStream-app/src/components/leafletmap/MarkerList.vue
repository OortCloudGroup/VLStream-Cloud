<template>
  <div class="marker-list">
    ...
    >
      <l-popup>
        <!-- 简单popup：只显示地址 -->
        <div v-if="popupType === 'simple'" class="simple-popup">
          <div class="popup-address">
            {{ worker.address || worker.name || '暂无地址信息' }}
          </div>
        </div>
        <!-- 工人信息popup：显示详细信息 -->
        <WorkerPopup v-else-if="popupType === 'worker'" :marker="worker" />
        <!-- 默认popup：保持原有行为 -->
        <WorkerPopup v-else :marker="worker" />
      </l-popup>
    </LMarker>
  </div>
</template>
...
const props = defineProps({
  ...
  // 控制popup显示方式：'simple' | 'worker' | 'custom'
  popupType: {
    type: String,
    default: 'worker'
  }
