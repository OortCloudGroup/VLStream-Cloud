...
export function getCustomLabel(subWidget) {
  ...
  if (subWidget.options.labelIconClass) {
    ...
    if (subWidget.options.labelIconPosition === 'front') {
      html.push(`<el-tooltip content="${subWidget.options.labelTooltip}" effect="light"><i class="${subWidget.options.labelIconClass}"></i></el-tooltip>${subWidget.options.label}`)
    } else {
      html.push(`<i class="${subWidget.options.labelIconClass}"></i>${subWidget.options.label}`)
    }
    html.push('</span>')
  } else {
    html.push(`<span title="${subWidget.options.labelTooltip || ''}">${subWidget.options.label}</span>`)
  }
...
const containerTemplates = {
  'data-table': (ctn, formConfig, vue3Flag = false) => {
    ...
    <el-table ref="dataTable" :data="formData.${ctn.id}.tableData" ${emptyAttr('class', ctn.options.customClass.toString().replace(/,/g, ' '))}
    ...
    if (ctn.options.showCheckBox) {
      return `<el-table-column type="selection" :width="${!ctn.options.showSummary ? 50 : 55}"  fixed="left"></el-table-column>`
    } else {
      return ''
    }
  })()}
    ...
    ctn.options.tableColumns.forEach((item, index) => {
      ...
      if (item.formatS === 'render' && !!item.render) {
        ...
        html.push(`<template ${vue3Flag ? '#default="scope">' : 'slot-scope="scope">'}`)
        html.push(`<span>{{formatterValue(scope.row, formData.${ctn.id}.tableColumns[${index}], scope.row['${item.prop}'])}}</span>`)
...
${ctn.cols
  ...
  'grid-sub-form': (ctn, formConfig, vue3Flag = false) => {
    ...
    rowHtml.push(`<div class="grid-sub-form sub-form-row" v-for="(subWidget,sfrIdx) in formData.${ctn.id}" :key="sfrIdx">`)
    ...
    const tableTemplate = `<div class="container-wrapper"  ${emptyAttr('class', tabClassAttr)}>
      <div key="${ctn.id}" class="sub-form-container">
        ...
        <div class="action-header-column">
          <span class="action-label">操作</span>
          <el-button round type="primary" size="small" class="action-button" @click="addSubFormRow('${ctn.id}')" title="新增行">新增${
  vue3Flag ? ' <Plus style="width:1em; height:1em;" />' : '<i class="el-icon-plus el-icon-right"></i>'
}
          </el-button>
        </div>
          ...
          ${ctn.widgetList
    .map(subWidget => {
      let subWidgetContainer = `<div key="${subWidget.id + 'thc'}" class="field-header-column"
           
            ${emptyAttr('class', (getLabelAlign(ctn, subWidget), subWidget.options.required ? 'is-required' : ''))}
    ...
    rowHtml.push(`<el-row class="sub-form-row" v-for="(subWidget,sfrIdx) in formData.${ctn.id}" :key="sfrIdx">`)
    ...
    ctn.widgetList.map((subWidget) => {
      rowHtml.push(`<div class="sub-form-table-column hide-label" key="${subWidget.id + 'tc' + rowId()}"  style="width: ${subWidget.options.columnWidth}">`)
...
export function buildContainerWidget(widget, formConfig, vue3Flag = false) {
  return containerTemplates[widget.type] ? containerTemplates[widget.type](widget, formConfig, vue3Flag) : null
}
...
function buildRadioChildren(widget) {
  let wop = widget.options
...
function buildCheckboxChildren(widget) {
  let wop = widget.options
...
function buildSelectChildren(widget) {
  let wop = widget.options
...
const elTemplates = {
  ...
  date: (widget, formConfig, parentWidget) => {
    const { vModel, readonly, disabled, size, type, placeholder, clearable, format, valueFormat, editable } = getElAttrs(widget, formConfig, parentWidget)
  ...
  'date-range': (widget, formConfig, parentWidget) => {
    const { vModel, readonly, disabled, size, type, startPlaceholder, endPlaceholder, clearable, format, valueFormat, editable } = getElAttrs(widget, formConfig, parentWidget)
  ...
  'file-upload': (widget, formConfig, parentWidget) => {
    const { disabled, uploadAction, withCredentials, multipleSelect, showFileList, limit, uploadTipSlotChild, fileUploadIconChild } = getElAttrs(widget, formConfig, parentWidget)
    let wop = widget.options
  ...
  input: (widget, formConfig, parentWidget) => {
    const { vModel, readonly, disabled, size, type, showPassword, placeholder, clearable, minlength, maxlength, showWordLimit, prefixIcon, suffixIcon, appendButtonChild } = getElAttrs(
      widget,
      formConfig,
      parentWidget
    )
  ...
  number: (widget, formConfig, parentWidget) => {
    const { vModel, disabled, size, type, showPassword, placeholder, controlsPosition, min, max, precision, step } = getElAttrs(widget, formConfig, parentWidget)
  ...
  rate: (widget, formConfig, parentWidget) => {
    const { vModel, disabled, rateMax, lowThreshold, highThreshold, allowHalf, showText, showScore } = getElAttrs(widget, formConfig, parentWidget)
  ...
  slider: (widget, formConfig, parentWidget) => {
    const { vModel, disabled, sliderMin, sliderMax, sliderStep, sliderRange, sliderVertical } = getElAttrs(widget, formConfig, parentWidget)
  ...
  switch: (widget, formConfig, parentWidget) => {
    const { vModel, disabled, activeText, inactiveText, activeColor, inactiveColor, switchWidth } = getElAttrs(widget, formConfig, parentWidget)
  ...
  textarea: (widget, formConfig, parentWidget) => {
    const { vModel, readonly, disabled, size, type, showPassword, placeholder, rows, clearable, minlength, maxlength, showWordLimit } = getElAttrs(widget, formConfig, parentWidget)
  ...
  time: (widget, formConfig, parentWidget) => {
    const { vModel, readonly, disabled, size, placeholder, clearable, format, editable } = getElAttrs(widget, formConfig, parentWidget)
  ...
  'time-range': (widget, formConfig, parentWidget) => {
    const { vModel, readonly, disabled, size, startPlaceholder, endPlaceholder, clearable, format, editable } = getElAttrs(widget, formConfig, parentWidget)
...
export function buildSubFormFieldWidget(widget, formConfig, subformWidget) {
  let wop = widget.options
  // const label = wop.labelHidden ? '' : wop.label
  // const labelWidthAttr = wop.labelHidden ? 'label-width="0"' : wop.labelWidth ? `label-width="${wop.labelWidth}px"` : ''
  // const labelTooltipAttr = wop.labelTooltip ? `title="${wop.labelTooltip}"` : ''
  ...
  const fwDom = elTemplates[widget.type] ? elTemplates[widget.type](widget, formConfig, subformWidget) : null
  // const isFormItem = !!widget.formItemFlag
  // const vShowAttr = wop.hidden ? 'v-show="false"' : ''
...
const genSubformScopedCSS = function(formConfig, vue3Flag = false) {
  ...
  .sub-form-container {
    ...
    ${vue3Flag ? ':deep(.el-row)' : ':deep(.el-row'}.header-row){
      padding: 0;
      display: flex;
    }
    
    ${vue3Flag ? ':deep(.el-form-item)' : ':deep(.el-form-item'}){
    margin-bottom: 0;
  }
  ${vue3Flag ? ':deep(.el-row)' : ':deep(.el-row'}.sub-form-row){
      padding: 0;
      display: flex;
  ...
  div.sub-form-action-column {
    ...
    ${vue3Flag ? ':deep(.el-button)' : ':deep(.el-button'}){
      font-size: 18px;
      padding: 0;
      background: #DCDFE6;
  ...
  div.sub-form-table-column {
    ...
    ${vue3Flag ? ':deep(.el-form-item__content)' : ':deep(.el-form-item__content'}){
...
const genGridSubformScopedCSS = function(formConfig, vue3Flag = false) {
  const cssTemplate = ` 
  .sub-form-container {
    ${vue3Flag ? ':deep( div.grid-sub-form.sub-form-row)' : ':deep()div.grid-sub-form.sub-form-row'} {
      display:flex;
      align-items: center;
      border: 1px solid #e1e2e3;
    }
...
export const genSFC = function(formConfig, widgetList, beautifier, vue3Flag = false) {
  ...
  let widget = widgetList.find(x => x.type === 'm-sub-form')
  if (widget) {
    scopedSubformCss = beautifier.css(genSubformScopedCSS(formConfig, vue3Flag), beautifierOpts.css)
  }
  widget = widgetList.find(x => x.type === 'grid-sub-form')
  if (widget) {
    scopedSubformCss = beautifier.css(genSubformScopedCSS(formConfig, vue3Flag), beautifierOpts.css)

    scopedGridSubformCss = beautifier.css(genGridSubformScopedCSS(formConfig, vue3Flag), beautifierOpts.css)
  }
