...
<script>
import { toRefs, reactive, onMounted, onBeforeUnmount, inject } from 'vue'
...
import { calcRules } from '~@/utils/funcItems'
import eventBus from '~@/utils/event-bus'

export default {
  componentName: 'FieldWidget',
  name: 'MCalcMethodWidget',
  components: {
    FormItemWrapper
  },
  props: {
  ...
  setup(props) {
    ...
    onMounted(() => {
      ...
      eventBus.$on('field-value-changed', otherField => {
        // 排除触发自身的事件
        if (otherField.id !== props.field.id && !otherField.id.startsWith('calcmethod')) {
          calcResult()
        }
      })
    })
    ...
    const calcResult = () => {
      let wgList = props.designer ? props.designer.widgetList : props.parentList
      // 赋值
      wgList.forEach(element => {
        if (globalModel.formModel[element.id]) {
          element.options.defaultValue = globalModel.formModel[element.id]
        }
      })
      data.fieldModel = calcRules(props.field.options.calcOptions.rules, wgList)
