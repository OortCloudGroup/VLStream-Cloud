<template>
  ...
  >
    ...
    >
      <template #button>
        <span>
          {{ field.options.numberOptions.type ===4 ? "%": field.options.numberOptions.moneyType }}
        </span>
      </template>
    </van-field>
  </form-item-wrapper>
...
export default {
  ...
  setup(props) {
    ...
    const formatter = (value) => {
      let numberOptions = props.field.options.numberOptions
      const digitNum = numberOptions.decimalLength

      if (value === null || value === undefined) return ''

      if (numberOptions.type === 1) return value.replace(/[^[0-9]/g, '')
