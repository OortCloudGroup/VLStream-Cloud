#!/usr/bin/env python3
"""Verify that public release SQL contains schema metadata but no exported data."""
...
REQUIRED_METADATA_TABLES = ALLOWED_INSERT_TABLES
INSERT_PATTERN = re.compile(r"^INSERT INTO `([^`]+)`", re.IGNORECASE)


def verify_sql(path: Path) -> None:
    """Reject data inserts and source-environment markers in the release SQL."""
    inserted_tables: set[str] = set()
    forbidden_markers = ("Source Server", "Source Host")
