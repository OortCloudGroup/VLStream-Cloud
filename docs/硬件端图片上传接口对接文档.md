...
平台生成 AWS V4 签名时，签名 Region 必须与 MinIO 服务端一致。若 MinIO 未配置
`MINIO_REGION` 或 `MINIO_REGION_NAME`，平台按 MinIO 默认值 `us-east-1` 签名；配置变更后
必须重新申请 `uploadUrl`，旧地址的签名不能修复或复用。

公网部署还必须配置 `VLSTREAM_DEVICE_MEDIA_PUBLIC_ENDPOINT`，例如
`https://vlstream.oortcloudsmart.com:2443`。平台会用该 HTTPS 地址直接参与 PUT 和预览 GET
的签名计算；Nginx 只负责原样转发，不能在签名生成后替换协议、主机名、端口、bucket 或对象路径。

设备管理由 WVP 统一负责。硬件仍调用本文中的 VLS 地址；VLS 会在内部向 WVP 校验
`deviceId`。设备只要已登记在 WVP 的“VLStream 协议”设备列表中即可，当前是否在线不影响
申请上传地址；未登记的设备会被拒绝。硬件端无需调用 WVP 接口。
...
| 字段 | 类型 | 是否必填 | 字段意思 |
| --- | --- | --- | --- |
| `deviceId` | String | 是 | 已登记在 WVP“VLStream 协议”设备列表中的设备编号 |
