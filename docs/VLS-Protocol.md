...
平台以接收时间记录最后心跳和最近上线时间，界面按浏览器本地时区显示 YYYY-MM-DD HH:mm:ss。首次在线、离线后重新在线时更新最近上线时间，连续在线心跳不刷新；历史在线设备缺少该字段时，以升级后首个合法在线心跳作为开始记录时间，不反推历史。离线遗嘱不刷新最后心跳。重复 messageId 或 sentAt 早于已接收快照的消息不改变状态。设备无需新增上线时间上报字段。
