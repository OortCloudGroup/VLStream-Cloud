...
`sql/upgrade` 包含历史兼容升级脚本和 v1.1.3 的 Flyway 迁移副本，供发布包审计和离线核对。正常升级只需部署新后端并启动容器，Flyway 会按版本顺序自动执行 v1.1.3 迁移；不要手工重复执行 Flyway 文件，也不要把全量初始化脚本导入已有数据库。所有升级前必须备份数据库。
