...
`sql/upgrade` 包含历史升级脚本、Hi3519DV500 OM 模型路径字段，以及 v1.1.2 的基础算法库预设。基础算法库脚本可重复执行，只会补充缺失的通用预设，不会覆盖已有算法。执行前必须备份数据库，并按文件名顺序人工审核后执行；不要把全量初始化脚本导入已有数据库。
