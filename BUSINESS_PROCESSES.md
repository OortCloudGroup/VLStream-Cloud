...
1. 具备 `vls:tunnel:manage` 权限的平台用户选择 WVP 业务 `deviceId`，VLS 向 WVP 校验设备，
   以当前登录租户创建或更新端点，并签发短期一次性激活码；设备不得提交 `tenantId`。
...
- 多租户设备所有权尚未在 WVP 原生设备表中建立可靠字段，因此无条件禁止多租户签发激活码；
- 该限制不是隧道协议本身不支持多租户，而是当前缺少可信的“租户 → WVP 设备”权威绑定；
  若直接采用当前登录租户或设备端自报租户，任一租户都可能抢占其他租户设备。启用前必须由平台管理员
  维护绑定，或从统一资产关系同步，并为既有设备完成归属迁移和冲突校验；
