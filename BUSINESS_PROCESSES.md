...
`[环境约束，2026-09-21]` 既有VM后端的平台校验、用户信息、租户列表/切换及事件上报通过
`http://192.168.50.15:32610/bus/`访问；部署Compose override使用对应SSO与事件上报环境变量覆盖。
浏览器的OortCloud公网入口保持不变，后续部署必须保留VM内网覆盖。容器连通性和VLS健康已验证，
真实用户登录换票及菜单行为仍需当前会话验收。业务身份和鉴权流程未改变。
...
`[已实现]` 登录页及登录后的平台顶栏提供统一语言选择；登录后入口独立于单租户/多租户平台顶栏，
两种租户模式均始终显示。支持简体中文、美国英语、拉美西班牙语、
阿拉伯语、德语、法国法语、日语、巴西葡萄牙语、俄语、韩语、印尼语和土耳其语。选择结果保存在
浏览器本地，刷新后继续使用；前端同时更新 HTML `lang`/`dir`、Element Plus 组件语言、字体栈、
日期数字格式和请求的 `Accept-Language`/`Content-Language`。阿拉伯语使用 RTL，其余语言使用 LTR。

现有历史页面继续保留中文源文案，由兼容词库在渲染时翻译公共导航、登录及通用操作文本；主框架、
递归侧边栏、工作台和事件管理常用界面词直接响应全局语言状态，业务数据和用户输入不参与翻译；新增页面
应直接使用全局 `vue-i18n`，不得继续新增散落的组件内语言状态。后端区域解析只接受受支持语言，
兼容 `pt_BR` 等旧下划线格式、语言列表及仅语言码；未知语言安全回退简体中文。后端尚无地区专用
资源时使用英语默认消息，中文由 `messages_zh_CN.properties` 明确覆盖。

多语言布局使用可收缩顶栏、内容感知菜单宽度、响应式分类侧栏和可换行工具栏。事件与设备表格保留
可读列宽并提供常显横向滚动条，不通过压缩单元格容纳长文本。分类、事件反馈/确认/详情、任务分配、
实时预览、设备详情/模型及固件/OTA 弹窗统一使用响应式宽度和内部滚动；非中文表单标签预留 180px，
窄屏自动改为上下排列。阿拉伯语同步镜像分页、弹窗、表格、表单和关闭按钮。

表格列标题直接在 Vue 渲染阶段调用响应式 `$tp(...)`，不再依赖 DOM 观察器。英文基线覆盖全仓库
267 个静态中文表头及已盘点页面文案，其他地区词库尚未审校的词条回退英文，不回退中文。`npm run lint:i18n`
检查表头是否仍有中文静态 `label`、12 个地区词库列是否完整，以及已迁移页面的界面词是否缺失。
`npm run audit:i18n` 盘点全部 Vue 模板的可见中文，`audit:i18n:strict` 要求归零；2026-09-20
首次审计的 1065 条、202 个文件已全部进入正式地区词库或英文回退基线，严格模板审计为 0。
该结果覆盖 Vue 模板静态可见文案；脚本拼接的运行时消息、后端/外部系统返回值及各非英语语种的
母语审校仍需分别验证，不得据此声明所有语种的所有运行路径均已完成。

语言切换只改变显示和地区格式，不改变租户、权限、设备、算法、训练、事件或工单数据。业务数据、
用户输入、设备名称和外部平台返回内容保持原值，不能当作界面词强行翻译。

### 4.4 租户平台标识

`[已实现]` “系统设置 → 平台设置”用于维护控制台左上角的品牌 LOGO。系统内置
VLStream Cloud 默认标识由后端固定提供，不作为租户数据保存，因此不能编辑或删除；每个租户可上传
PNG/JPG/JPEG 图片、维护描述、保留多个自定义方案，并选择一个当前生效。未选择自定义方案、当前方案
被删除、图片加载失败或配置接口不可用时，控制台自动回退系统默认标识。

自定义方案及当前生效状态按 `tenant_id` 隔离，普通用户只读取当前租户的生效标识，管理操作由
`system:platformLogo:*` 权限控制。图片正文进入现有 OSS，配置表仅保存 OSS 对象 ID；替换或删除方案时
同步清理对应对象。上传限制为 2 MiB、PNG/JPG/JPEG、最大 4096×4096，前后端均做校验。

验证边界：Flyway 迁移检查、Java 8 定向测试、前端多语言门禁及生产构建已通过；本地浏览器完成默认项
不可编辑/删除、新增、编辑、切换、删除当前项后回退以及刷新保持生效的交互验证。
...
#### 5.3.2 IPC 原厂 Web 后台远程管理（2026-09-20）

`[开发联调中]` 用户入口为“设备管理 → VLStream 协议 → 设备详情 → 打开管理后台”。
首期仅支持运行在 VLStream 自有 IPC 本机的 Agent，固定转发该设备回环地址上的 HTTP Web 服务，
不允许 Agent 访问同一局域网的其他主机。IPC 原厂账号密码继续由 IPC 自身校验，VLS 不保存、不代填。

参与者与边界：

- WVP 继续持有权威的 VLStream 设备业务 ID、行 ID、在线状态和视频能力；
- VLS Tunnel 控制面持有租户设备绑定、Agent 状态、内部路由期望和短期访问会话；
- IPC Agent 管理本机 rathole Client，不决定租户、公网端口或公网 URL；
- rathole 控制器把服务映射到与 HTTPS 网关同一网络命名空间内的回环端口；
- HTTPS 网关校验短期访问会话后转发 IPC Web；公网不能直接访问 rathole 映射端口。

主流程：

1. 具备 `vls:tunnel:manage` 权限的平台用户选择 WVP 业务 `deviceId`，VLS 向 WVP 校验设备归属
   与当前登录租户一致后创建或更新端点，并签发一次性激活码；Agent 注册接口不接受 `tenantId`。
2. IPC Agent 使用激活码注册，换取只返回一次的 Agent Token 及按端点/配置版本派生的
   rathole Service Token。数据库只保存激活码和 Agent Token 的 SHA-256，不保存其可用明文。
3. Agent 独立上报 Agent、隧道和 IPC 本地 Web 状态。该心跳不覆盖 WVP 的
   `deviceBiz/state`，设备在线与远程管理可用是两个不同状态。
4. 受信 rathole 控制器读取期望路由，生成并应用服务端配置，映射端口仅绑定
   `127.0.0.1`，再回报相同配置版本的 `APPLIED` 或 `FAILED`。
5. 具备 `vls:tunnel:access` 权限的用户只有在 Agent 心跳新鲜、隧道在线、本地 Web 可用、
   路由版本已应用时才能创建短期访问会话。
6. HTTPS 网关使用独立内部凭据解析访问令牌，只获得回环上游和过期时间；浏览器继续完成
   IPC 自身登录。

2026-09-22 会话时长调整：远程 Web 访问会话默认空闲 10 小时（36000 秒）过期，首次打开和后续
HTTP 请求（包括刷新及后台轮询）均由网关调用内部续期接口，后端再次校验会话、归属与设备可用性，
有效时将数据库到期时间和浏览器 Cookie 延后 10 小时。入口令牌仍只能兑换一次，过期或吊销后不可续活。
仅保持页面打开不会自行续期；WebSocket 在 Upgrade 请求时续期，后续帧不单独延长会话。
`VLSTREAM_TUNNEL_ACCESS_SESSION_TTL_SECONDS` 可在 60～36000 秒范围内配置；两套 Compose 均透传该值。
此时长只控制远程访问会话，不改变一次性设备激活码的 10 分钟默认期限，也不改变 IPC 原厂登录的会话期限。
内部到期时间采用带 UTC 偏移的 ISO-8601 格式，不受控制台本地时间格式影响。
本次续期及租户改造尚未部署到运行中的 VLS/WVP/网关，数据库迁移只在隔离测试库验证。
验证：VLS 24 项、WVP 29 项定向测试及 Go 网关测试通过；MySQL 8.4 验证新增迁移的幂等性、
默认租户兼容和 A/B 查询隔离，初始化工具与 Java 校验器的签名测试向量一致。

核心状态：

- 端点期望状态：`ENABLED / DISABLED / REVOKED`；
- Agent 状态：`WAITING_FOR_AGENT / REGISTERED / CONNECTING / ONLINE / DEGRADED` 等；
- 路由状态：`PENDING / APPLIED / FAILED`，并与 `configGeneration` 严格对应；
- 访问会话保存哈希令牌、创建用户、过期时间、首次打开和吊销时间。

失败与安全规则：

- 2026-09-22 按用户确认采用设备初始化租户：WVP `V1_2_9` 增加设备归属，首次上报未携带租户时
  归配置的默认租户，已有设备漏报时保持当前归属。显式租户须附带初始化环境签发的设备/租户 HMAC
  绑定凭据，心跳不能改变已有归属；MQTT 仍依赖每设备认证和自身 Topic ACL。
- WVP 登录上下文保留经上游校验的租户，VLStream 列表、详情、视频源、预览、固件及分类设备关联
  按归属校验；历史空归属只对默认租户可见。分类结构名称及其他设备协议仍沿用既有行为，不声明已全局租户化。
- VLS 读取 WVP 返回的租户并与当前操作者核对，不再统一禁止多租户激活；旧 WVP 缺少字段时仍保留
  配置的默认设备租户兼容。两项目默认设备租户必须一致，切换配置不用于迁移设备。
- 激活码过期、设备不匹配、被其他 Agent 使用或端点吊销时注册失败，不降级为匿名；
- 内部控制器和网关密钥为空或不足 32 字符时失败关闭；
- rathole 使用 Noise 加密，Service Token 与 Agent Token 分离；任何 token、Cookie、IPC 密码和
  完整临时 URL 均不得写日志；
- 停用或吊销端点同时撤销未过期的浏览器会话；
...
Go 8 项测试、MySQL 8.4 迁移实跑、迁移路径检查、sidecar 镜像/非 root 只读运行健康检查、
Compose 解析和前端生产构建通过。开发数据库已实际执行迁移；本地最新版 VLS、sidecar、
模拟 IPC Agent 和 Noise rathole Client 完成真实链路验证：设备详情显示远程管理可用，
短期入口返回 303 并签发会话 Cookie，网关成功代理到模拟 IPC Web，一次性入口令牌复用返回 401。
联调后已吊销模拟 Agent、删除模拟容器，控制面活动路由恢复为 0，sidecar 保留运行供后续板端联调。

运行联调同时修复了两项兼容问题：Agent `reportedAt` 接受标准 UTC ISO-8601 的纳秒精度；
登录 SM2 密文首字节自然为 `04` 时兼容有/无椭圆曲线点前缀两种输入。内部访问令牌改为请求头传递，
认证、Agent 注册和会话解析接口的通用请求日志不再记录敏感参数。

`[待设备/部署联调]` 仍需把签名密钥、控制 Token、网关 Token 和 Noise 密钥持久化到部署密钥配置，
配置正式域名的通配 DNS/TLS，并用真实 IPC Agent 验证原厂登录、静态资源、WebSocket、重启/断网恢复、
凭据轮换和 24 小时稳定性。完成这些证据前，只能声明平台开发环境链路可用，不能宣称生产和真实板端已完成。
...
#### 四类标注、智能标注与正式训练（2026-09-22扩展）

`[已实现，待正式环境更新与GPU验收]` 用户确认扩展到图像分类（单标签）、物体检测、实例分割、
语义分割，并打通各类人工标注、智能标注、确认、数据集版本、训练目录和正式训练。
数据集管理开放四类；检测保留原标注页，其余类型进入`/dataset-annotation`。
分类保存整图类别；实例逐对象保存二值PNG掩膜（保留孔洞、断开区域和重叠）；语义分割逐像素归类，
不允许同一像素多类别或存在未归类像素。画笔、橡皮、撤销、标签映射和填充用于人工修正。
人工编辑采用版本摘要拒绝并发覆盖；不兼容的旧标注经用户确认替换前自动保存版本。

四类均支持主动学习和指定模型，复用GPU整卡队列。类别自动按名称匹配，未知类别仍需人工选择。
显式“一键确认剩余预测”进入`CONFIRMING`，后台逐图事务处理并保存游标，重启后可继续；低置信度、
空预测、标签未匹配或人工编辑冲突等留待处理。取消保留已确认数据。完成保存结果版本，不暗中采用剩余预测。
每轮训练只使用正式确认标注；分类/语义门槛按每类图片数，检测/实例按对象数。状态与轮次来源可追溯。

训练数据分别输出分类索引目录、检测TXT、实例原始二值mask JSON、语义类别索引PNG。
YOLO实例分割采用直接加载像素掩膜的Dataset，不以外接框代替；语义训练使用DeepLabV3 ResNet50。
各类生成独立PT与指标文件；非检测正式训练可保存模型并再次用于智能标注，不自动套用检测专用板端转换。
系统预置模型首次使用需官方权重下载/预置缓存，GPU容器需兼容PyTorch/Torchvision/Ultralytics及CUDA。
数据库、对象存储和GPU目录的责任沿用原链路；迁移022接续021，不重写已执行迁移。

验证：35项Java回归（含四类真实MySQL事务、版本、训练文件和批量确认）通过；四类真实CPU小样本训练与
推理均生成PT及对应输出。合成数据验证可执行性，不代表业务精度；浏览器记录在`codex/smart-four/`。
正式库迁移、服务部署和真实GPU尚未执行。详细限制与使用方式见`docs/dataset-import/SMART_ANNOTATION.md`。
本段取代上文历史记录中的“只开放物体检测”限制，其他导入来源/视频切图规则不变。

#### 物体检测智能标注初版（2026-09-22，后续范围以上段为准）

`[已实现，待业务库迁移、服务更新及真实GPU验收]` 用户从算法训练平台“智能标注”或数据集详情进入
`/smart-annotation`，创建“主动学习 / 指定模型”任务。仅支持物体检测，视频沿用先切图再标注流程。
指定模型复用当前租户已有PT模型或成功主动学习轮次；主动学习使用已确认样本训练模型，再按预测置信度
排序优先展示难例。首轮默认每类10个标注框且未标图片至少101张，最多4轮，门槛可通过服务端配置调整。

每轮冻结既有数据集版本，复用现有SSH/Docker GPU队列，正式训练和智能标注整卡串行。
状态为排队、准备数据、计算中、待确认、完成；失败支持基于同一输入重试，取消请求由调度器处理本轮容器。
智能标注模型不会触发正式训练的自动发布或模型转换。VLS/MySQL保存任务、轮次、预测和确认状态；
当前对象存储持有图片，GPU主机持有冻结输入、计算日志和模型文件，模型SHA-256作为轮次来源记录。

预测保存在独立表中，不计入正式已标注图片。用户可改单图类别/框、补框、删框或显式批量确认；未知模型类别
必须选择数据集标签。确认通过原标注保存服务，更新图片进度、使训练目录失效；重复确认幂等，批量冲突整批回滚。
确认前校验租户、图片及标签归属，拒绝覆盖已经人工标注的图片。跳过保留未标注状态，不新增负样本训练语义。
...
#### 标注项目删除与训练产物保留（2026-09-16）

`[已实现，待业务库迁移及后端更新]` 算法标注列表的单条、批量及兼容删除入口统一执行项目清理。
用户确认后，后端先按可信租户锁定项目；排队、运行、转换中的训练、活动容器及上传/导入任务会阻止删除。
训练入队、模型转换、标注编辑及训练数据生成使用相同项目锁，清理开始后的项目不能继续编辑、导入或训练。
模型转换使用独立占用记录覆盖完整 ONNX→OM→RKNN/INT8 链路；进程异常退出后保留占用，
须确认远端转换已停止再由运维解除，不因 ONNX/OM 已完成而提前删除校准图片。
批量删除逐项目完成；中途失败时前端刷新列表，已完成项目不回滚，剩余项目可重试。

在删除任何文件前，保存持久清理清单并把既有模型原始类别 YAML 保存为独立模型元数据快照，
以租户、训练 ID 和模型路径匹配；缺失原始类别信息时停止删除，不从当前可编辑类别推测。
模型下发优先读取快照并验证 SHA-256，故删除训练输入后仍可下发已有模型。
保留训练任务、模型记录及 PT/ONNX/OM/RKNN 文件；已删除数据不能用于重新训练或依赖原数据的转换。

清理范围包含项目样本、类别、标注实例、版本快照、视频切图来源、导入任务/分片登记、独占媒体及对应 OSS 登记，
以及 GPU 服务器上由数字项目 ID 确定的 `datasets/vls/annotation_{id}` 目录。
其他项目及其历史版本引用的对象保留；数据来源连接配置不属于单个项目，不删除。
远端清理不使用任意客户端路径、不跟随符号链接，遇到模型或未知类型文件时停止。
清单固定存储桶、访问位置和训练服务器身份；配置变化时拒绝继续，防止重试清理到其他位置。
清理也检查曾生成但当前路径已失效的项目目录，因此完整删除依赖对象存储和训练服务器可访问。

外部文件删除不能由数据库事务回滚。文件清理失败时保留清单和 `FAILED` 状态，项目保留在列表用于重试，
但不可继续使用其数据；界面明确提示可能部分清理，不返回全部成功。重试对已消失的对象/目录幂等处理。
文件清理全部完成后，移除关联业务记录并逻辑删除项目主记录；清理审计和模型类别快照独立保留。
新增表由 `V1_2_0_017__dataset_cleanup_and_model_classes.sql` 管理。

`[验证]` Java 8 编译及隔离 MySQL 删除/重试测试通过，覆盖活动任务、跨租户、共享图片、存储变化、
模型类别信息缺失、失败重试及模型路径保留；远端文件操作使用替身，未删除真实 GPU 或业务存储数据。
当前业务库尚未执行本次迁移，运行中的旧 JVM 未更新，不能将源码测试当作新删除接口已上线。

`[已实现]` 标注图片列表一次解析存储客户端，不再按每张图片反复初始化和重试；图片加载有明确的加载中、
失败和重试状态，失败不会显示“暂无图片”。浏览器已验证错误与重试入口，前端生产构建通过。
`[运行恢复，2026-09-16]` 存储端点曾返回空响应，旧 JVM 卡在存储桶检查。确认该 IP 为本机，
MinIO 容器内部健康检查为 200，但 Windows 发布端口无有效 HTTP 响应；仅重启 `minio-local` 后恢复。
原配置上传/读取/删除独立测试对象返回 200/200/204，浏览器原有图片及标注标签重新显示。
未改存储端点或迁移用户图片，未重复提交用户 ZIP；新的删除逻辑仍待迁移及后端更新。

### 7.3 创建和执行训练

#### 训练配置、数据预检与后台模型保存（2026-09-16）

`[已实现，待后端更新及迁移]` 算法训练的开始/重新训练配置提供“默认配置”和“自定义参数”。
默认使用任务轮数、batch=16、imgSize=640；自定义允许设置轮数、批大小及输入尺寸。
前后端统一使用 `imgSize`，实际参数和自动保存选项在入队事务中保存，不再依赖独立未认证配置请求。
轮数范围1至10000、批大小1至256、尺寸320至1280且为32的倍数；不接受任意shell参数。
重新训练已有模型的任务须再次确认，使用新的独立输出目录和日志；原算法模型记录及物理模型文件保留。

开始前锁定项目及任务，拒绝活动训练/转换和待保存的上一轮完成结果；只支持目标检测算法与目标检测数据集。
后端使用训练服务器配置的Python环境执行只读校验，核验基础模型文件存在、YAML类别连续且一致、
图片可读、每图标签存在、类别范围、有限归一化框坐标、训练/验证集合非空且各有有效标注，
用SHA-256拒绝跨集合相同图片。仅支持生成器的images/labels目录布局；不支持的布局明确拒绝。
预检最多180秒，失败不会入队或把上一轮成功状态改成失败。预检不证明标签语义正确或真实场景效果达标。

训练输出记录为任务ID加随机轮次的独立目录，命令明确指定project/name；只读取本轮best.pt，
不再搜索“最新train目录”。历史任务保留既有模型路径，缺少已记录路径时不猜测其他任务产物。
入队与配置写入共同提交；新轮开始清除任务旧产物指针，独立模型记录不变。

“训练完成后保存到算法模型”写入 `vls_training_publication` 持久任务，后台定时处理，不依赖浏览器在线。
等待PT就绪、ONNX/OM进入终态且完整转换占用结束，保存所有成功格式的路径；转换失败时仍可保留有效PT。
状态为PENDING/COMPLETED/FAILED/DISABLED；创建模型和完成状态在同一事务内提交，相同任务/产物避免重复。
失败自动重试3次后展示重试入口；关闭页面或服务重启不丢失待办。保存模型记录不等于设备部署。
新增表由 `V1_2_0_018__training_auto_publish.sql` 管理；页面后端就绪探测失败时禁止启动，避免新旧版本混用。

`[验证边界]` Java 8定向测试、Python真实小图片/标签预检、前端参数断言、构建与定向ESLint；
自动保存事务使用独立MySQL测试库验证回滚/重试/幂等。浏览器核对原轮数恢复、模式及旧后端启动保护。
没有启动真实GPU训练，未应用本次业务库迁移或重启业务JVM；实际训练及模型精度仍待更新后验证。

2026-09-12 表单提示修复：新增/编辑训练任务的字段校验失败时停止提交，保留弹窗和字段级错误，
不再将Element Plus校验结果当作接口异常提示“未知错误”。字段规则和实际接口失败提示保持原有语义。
生产浏览器用单字符任务名及未选必填项验证：字段错误正常显示，新增POST为0，无“新增失败”弹窗。

2026-09-12 删除契约修复：单条/批量删除继续使用 `is_deleted` 逻辑删除，不移除训练数据或模型文件。
XML批量参数名与Mapper的 `@Param("ids")` 保持一致；空数组/列表生成恒假条件，不产生全表更新。
`TrainingDeleteMapperTest` 通过真实MyBatis映射加载、ParamNameResolver与PreparedStatement参数绑定
覆盖单条、数组批量、列表批量及空输入；更新服务前运行 `pwsh -File tools/check-training-persistence.ps1`，
保留 `codex/training-persistence-check/tests.log`。该门禁需通过后再进行更新后的接口验收，不能只做跳过测试的打包。

2026-09-12 列表兼容性修复：历史训练记录的关联算法缺失或不可见时，列表保留该任务并显示
“关联算法不存在”，不再因读取算法名称发生空指针而使整页500。训练启动仍须验证真实算法，
本修复不自动修正任务归属、不删除历史任务，创建→排队→训练的业务流程保持不变。
前端新建和编辑训练时算法/数据集ID保持字符串，生产旧页面中的Number转换同步移除。
对本次5条已确认因Number舍入丢失精度的pending记录，以同租户唯一有效算法和数据集核验，
通过V1_2_0_015限定任务ID及旧值修复关联；先备份、事务回滚演练，再执行并验证幂等。
生产列表HTTP200、第一页5条修复任务与第二页4条旧任务均已浏览器验证；没有自动启动训练。
...
7. 通过所选模型 PT 所在训练目录的 `args.yaml` 中 `data` 读取训练数据集 YAML，校验非空类别、连续编号及 `nc` 一致性；缺失或无效时拒绝该次下发，不取当前可编辑数据集替代。
8. 每台设备创建独立下发任务，保存类别 YAML 的 UTF-8 内容快照、文件名、字节数及 SHA-256；生成模型和类别文件各自的短时签名地址，通过同一 MQTT `modelDeploy` 发布。
9. 设备按协议下载、校验并成对切换模型和类别映射，通过 MQTT 回执更新任务状态。新任务成功回执必须同时匹配模型与类别文件 SHA-256，缺失或不匹配按失败处理；历史任务仍保持原约定。

2026-09-14 类别文件下发：新增 `classFileUrl/classFileName/classFileSize/classFileSha256`。
类别文件原本已由数据集生成流程创建，`names` 的编号不能重新排序或跳过空项。
VLS/MySQL 保存下发快照；`GET /vlsModelDispatch/public/{requestId}/classes` 使用独立资源签名
返回快照，与模型地址共用过期时间，不依赖用户登录或下载时的远端文件状态。
过期后用户重新下发；传输完成不代表部署成功，最终以设备匹配双哈希的回执为准。
设备端需实现双文件校验、一起生效及失败一起回滚，不能忽略新字段后沿用旧类别表。

`[边界]` 训练来源要求保留原始 `args.yaml` 和它引用的 YAML；新生成的版本目录不覆盖。
历史目录若被人为覆盖，必须先核对并恢复与模型一致的原始类别文件，平台不能从当前数据集猜测。
新增 Flyway `V1_2_0_016__model_dispatch_class_file.sql`；Word 与 Markdown 协议第 4.1 节同步更新。
`[验证]` Java 8 编译及 13 项定向测试通过，覆盖来源路径、中文字节、非法类别、缺失文件、
签名隔离、任务快照及双哈希回执；迁移路径检查通过。未更新运行中的服务或业务数据库，
未向真实设备下发，固件双文件激活与回滚仍待联调。
...
2026-09-17 主动安全状态读取环境修复：`oort_task_event.status` 为tinyint(1)，业务值1/2分别为
已完成/正在处理。JDBC连接必须设置 `tinyInt1isBit=false`，避免getObject将2读成Boolean true，
兼容服务数字转换失败后返回0，页面误显示未知状态。发布内置/外部数据库Compose模板已补齐该参数。
生产只更新VLS连接参数并重建backend，不修改事件数据；真实JDBC对照验证及线上第一页10条事件
恢复“正在处理”已通过，事件业务流程保持不变。
...
### 8.1 设备详情模型管理（2026-09-16）

入口：“视频汇聚 → 设备管理 → VLStream协议 → 详情 → 设备运行模型”。
平台用户点击“模型下发”从右侧抽屉选择本系统已保存的自主训练模型，支持搜索、实际算法分类、分页、跨页多选及移除。
自主训练模型列表来自 `/vlsModelDispatch/models`，仅包含有训练来源的模型记录；
缺少 OM 或原始 PT 信息的模型展示为不可选。确认后逐项调用 `/vlsModelDispatch/models/{modelId}/dispatch`，
每项独立返回任务编号；成功、失败或结果未确认分别展示，已尝试的项目不自动重复提交。
提交期间禁用修改和关闭；关掉重开抽屉清空选择，搜索及翻页保留当前选择。下发目标固定为打开详情的当前设备。
按选定模型记录保存的 OM/PT 路径与训练编号准备文件和类别快照，即使训练任务后来重新训练，也不改发新版本。
不回退到“该算法最新可用产物”，但继续复用既有 `modelDeploy` 与类别文件签名、任务持久化和回执处理。
“刷新”经 VLS `/vlsDeviceModels` 向 WVP 校验设备在线，再发 `aiBiz/modelQuery`，按设备回执展示全量列表。
“删除”经二次确认发送 `aiBiz/modelDelete`，携带设备模型 `modelId`；设备负责停止推理并卸载模型及类别文件，平台模型库与训练产物保留。
设备仍是实际模型状态的权威来源，WVP 继续持有心跳快照，VLS 不新建模型库存表。
本次查询结果保留在当前详情中，关闭后重新打开先显示 WVP 心跳快照；设备后续心跳须反映模型变更。

查询、删除状态为等待设备回执 → 成功/设备拒绝/超时。回执必须匹配请求 messageId、设备和子业务，
删除还需匹配 modelId 与 SUCCESS；15 秒无回执不作成功处理，不自动重发删除。失败保留原列表并提示核实。
离线禁用设备刷新、删除和提交下发，但允许通过“查看模型”浏览自主训练及 Model Hub 目录，登录后可下载云端文件；
旧版单模型查询仍可查看，但没有 modelId 时禁用删除；切换设备或关闭详情后忽略迟到的页面响应。
登录鉴权沿用 VLS，设备目录和在线状态来自 WVP，通信依赖 MQTT 与设备固件。

验证：Java 8 定向测试、前端生产构建、临时 Vue ESLint 检查、浏览器入口/算法列表/离线禁用已完成，证据在 `codex/device-model-management/`。
本地旧 JVM 的新查询接口实测返回 404，尚未更新后端或验证实机查询/卸载；协议定义不表示固件已经实现。

2026-09-17 模型选择抽屉验证：Java 8 下发测试覆盖保存版本的路径及类别来源、缺少模型/OM禁止下发，前端构建及定向 ESLint 通过。
隔离浏览器验证跨页多选、分类/搜索保留选择、移除、不可用 OM 禁选、逐项提交和部分失败；模拟接口不连接设备。
本地 Vite 缓存未刷新时仅重启了 3000 前端开发服务，IDEA 后端需重新加载新接口；未向真实设备下发模型。

2026-09-18 模型选择查询兼容修复：当前算法表为 utf8mb4_unicode_ci、模型表为 utf8mb4_0900_ai_ci，
联表 tenant_id 比较导致 MySQL 1267。查询显式统一租户比较及描述回退表达式的排序规则，保留租户隔离，
不改变表结构、业务数据或模型下发流程。真实库分页/搜索及连接级临时表隔离回归通过；
保留启动参数和环境重启本地 VLS 后端后，浏览器接口 HTTP 200 / code 200，20 条模型、12/8 两页和搜索分类验证通过。
证据：`codex/device-model-collation/`。未执行真实模型下发。

2026-09-18 Model Hub 浏览接入：右侧抽屉新增独立 Model Hub 页签，VLS `/vlsModelHub/models`
代理测试部署服务 `aiModel/v1/modelList`，支持真实数量、搜索和分页。公共列表使用服务端游客请求头；
配置从 `VLS_MODEL_HUB_BASE_URL/GUEST_TOKEN/APP_ID/SECRET_KEY` 环境变量读取，源码不保存真实凭据。
按用户明确的最终边界，查看公开模型、分类及文件目录无需登录 OortCloud；所有浏览请求固定使用服务端游客身份，
已登录个人身份不会覆盖游客配置。categories/space 上游既带游客请求头，也将游客 accessToken 放入 JSON，
此组合已实测分类及详情200，历史模型space返回4101（没有文件空间，而非未登录）。
下载才要求独立 OortCloud 身份，VLS 拒绝空身份和游客下载，上游继续校验用户权限；
下载请求体携带用户accessToken。单文件使用上游downloadFile、path必填，不误用分支ZIP的download。
分类来自真实categories返回，公共列表按category精确筛选；无分类时仍可浏览列表。
下载不回退游客身份，不直接使用公开file_path绕开授权；下载错误JSON不保存成模型文件。
单租户使用独立 OortCloud 会话，多租户使用平台会话，不把 VLS 本地管理员登录当作 OortCloud 登录。
浏览失败显示游客配置或服务错误，不要求用户登录；仅点击下载且未登录才进入OortCloud登录。
浏览与个人会话变化解耦，下载中身份变化则丢弃结果；文件下载期间禁用切换和关闭。

历史模型缺少repo_owner/repo_name时明确提示尚未建立模型文件空间，不回退到FastDFS公开地址直接下载。
测试服务当前公开目录仅1条gpt-3.5-turbo，其file_path可访问，但Range读取前16字节确认是PNG图片，
并非可部署OM。接口未提供设备兼容格式、类别文件配对及双哈希契约，因此Model Hub设备下发保持禁用，
不会把图片或任意远程文件当作设备模型。此前categories只带游客请求头、未按DTO传请求体时的4004，不代表游客不可浏览。
Java 8定向测试及前端构建用于验证请求契约；本地进程已注入游客环境配置，浏览器实测Model Hub(1)及登录提示。
用户已确认测试服务接受现有生产OortCloud登录，沿用现有入口，不修改全局认证配置。
浏览器恢复后已验证Model Hub真实列表与历史文件提示，并成功跳转到现有OortCloud统一登录页；
VLS本地会话不等于OortCloud会话。后续真实OortCloud登录回调与会话识别已通过，刷新清理URL后仍能识别。
个人身份调用测试分类接口的4004已记录，但浏览流程不再使用个人身份，因此不会阻断游客查看或诱导重复登录。
唯一历史PNG模型无Gitea文件空间，文件弹窗正确提示；实际授权下载仍需可用仓库文件样例及上游用户权限验证。
不再监听storage事件重载公共目录，避免无关存储变化引发重复查询。
游客配置仅存在当前进程环境，重新从IDEA启动时需要配置上述环境变量。证据：`codex/model-hub-integration/`。
...
`[已实现]` 单租户已登录用户从顶部入口发起 OortCloud 授权时，回调令牌只写入独立
OortCloud 会话，不换取或覆盖当前 VLS 本地会话。回调兼容 `accessToken`、`access_token`
和 `token` 三种参数；前端以一次性短时标记阻止 Vue Router 把已消费的回调再次识别为
VLS 平台登录，并清理地址栏认证参数。登录页主动发起的平台授权仍按原流程换取 VLS
本地会话。生产包隔离浏览器回归已验证：原 VLS 用户保持不变、OortCloud 回调被消费、
顶部 OortCloud 入口持续可见，测试证据位于 `codex/oortcloud-auth-callback/`。
...
- 2026-09-21：新增按租户隔离的平台标识方案管理，系统默认 VLStream Cloud 标识不可编辑删除；支持图片上传、方案切换、当前项删除回退和顶栏即时更新。

- 2026-09-20：将全仓库 Vue 模板可见中文审计从 1065 条/202 文件降为 0，完成视频汇聚、算法训练/数据、WVP、流程/工单、系统与通用组件的英文基线；新增 `audit:i18n:strict` 零残留门禁。其他 11 个非中文语种未审校词条仍回退英文。

- 2026-09-20：参考 Vue Vben Admin、Element Plus ConfigProvider 与 Intlify lint 方案，将全仓库 67 个 Vue 文件、267 个静态中文表头改为渲染期 `$tp(...)`；新增英文表头基线和 `lint:i18n` 门禁，补充工作台引导卡片响应式布局及 ISUP、EHome、数据集流程、算法训练介绍英文基线。

- 2026-09-20：修复长文本语言和阿拉伯语 RTL 下的顶栏、分类侧栏、事件工具栏及表格错位；补齐 Teleport 子弹窗监听、响应式宽度、长表单标签、设备时间/时长/能力格式和事件/设备弹窗界面词。德语与阿拉伯语页面、分类弹窗、事件反馈和事件确认弹窗完成浏览器验证。

- 2026-09-20：完成 VLStream IPC 原厂 Web 后台远程管理平台侧控制面、sidecar、Compose 与
  设备详情入口，并以模拟 IPC 完成真实 Noise 隧道、短期会话、Cookie 代理和令牌防复用联调；
  模拟端点已吊销清理。正式通配 DNS/TLS、持久化密钥和真实 IPC 原厂页面仍待板端与部署联调。

- 2026-09-18：修复单租户登录后不显示语言入口的问题，将切换器从多租户平台头拆为全局顶栏组件；补齐主框架、工作台和事件管理界面翻译及覆盖断言，保留设备名称、位置、标签等业务数据原值。生产构建和已登录页面中英文切换通过。

- 2026-09-18：控制台新增 12 种地区语言统一切换、Element Plus 地区包、请求语言头、地区格式和阿拉伯语 RTL；兼容历史硬编码页面公共词，后端补齐安全的区域解析与英语默认回退。前端构建、登录页中/德/阿浏览器切换和 Java 8 区域解析测试通过。

- 2026-09-16：统一训练参数及模式语义，新增数据内容预检、独立轮次输出/日志和持久后台模型保存；业务库迁移与运行环境更新待完成，未启动真实训练。

- 2026-09-16：标注项目删除新增持久清理清单、活动任务/转换占用保护、共享对象保护和模型类别快照；图片加载改为一次获取存储客户端并提供失败重试。隔离 MySQL 与定向测试通过，业务库迁移、后端更新和存储服务恢复待完成。

- 2026-09-14：模型下发携带训练来源类别 YAML 快照和独立签名下载信息，新任务成功回执校验模型及类别双哈希；13 项 Java 8 定向测试、迁移路径检查通过，设备端适配和实机联调待完成。
