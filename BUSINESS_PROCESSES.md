...
按指数退避自动重连，最多尝试 5 次，之后保留人工“重新连接”入口。
...
选中后可用删除按钮或 Delete 键直接移除；点击“保存当前标注”才写入后端。
...
`[已实现]` 用户从“算法仓库 → 算法训练平台 → 数据和样本管理”进入 `/data-management`，
后端入口为 `/vlsData/projects`。项目沿用 `vls_algorithm_annotation` 的 ID；
图片和视频沿用 `vls_annotation_image` 保存元数据，类别与标注继续使用原标注表。
用户可直接进入原标注编辑器，无需复制已有标注任务。

正常流程：

1. 创建或编辑项目名称、租户内唯一编号、项目类型、说明、标注类型和标注规则。创建时间由服务端记录；
   已有标注时禁止直接更换标注类型。类别在原标注页维护。
2. 批量导入 JPG/JPEG、PNG、BMP 图片或 MP4/MOV 视频，记录来源、稳定对象键及 SHA-256。
   图片执行格式匹配、完整解码、像素限制和清晰度检查；视频检查容器边界、媒体数据及视频轨道，
   内容质量由人工播放抽检。相同项目中的相同文件内容自动去重。
3. 按名称、标签/标注类别、来源、媒体类型、质量、标注状态和训练/验证划分查询样本。
   支持预览、修改名称/来源/标签、批量质量检查、随机抽检、人工结论、剔除及逻辑删除。
4. `pending`（待审核）可经人工操作变为 `accepted`（合格）、`low`（低质量）或 `excluded`（剔除），
   也可恢复待审核。自动发现损坏、低清晰度或标注问题时标记低质量并保留原因；
   低质量/剔除样本退出划分，视频不进入原图片标注页和图片训练数据集。
5. 对有标注且非低质量/剔除的图片按比例随机、按首个类别分层或指定验证样本 ID 划分。
   默认训练/验证为 80%/20%，固定种子可复现。两个集合必须非空，同一文件内容不能跨集合；
   小类别按整数样本取整，因此实际比例可能有差异。页面显示数量和每个标注类别的样本分布。
6. 用户手动保存版本，或划分时自动保存。版本快照包含样本成员、对象键、元数据、质量和划分、
   类别、标注实例及标注规则。重新划分已有数据集前保存备份；回退先保存当前版本，再在事务内恢复
   目标版本，最后生成“回退至 Vx”记录。支持版本详情、与当前数据/其他版本比较，以及版本 ZIP 导出。
7. 样本 ZIP 导出包含真实媒体和 `vls-samples.json`；支持按筛选或已选记录导出，并重新导入兼容项目。
   普通媒体 ZIP 也可导入；原有 YOLO ZIP 仍使用标注页入口。归档导入先校验所有文件和引用，
   对象存储完成后在一个数据库事务内登记，失败回滚登记并清理本次暂存对象，重复导入不覆盖已有标注。
8. “生成训练目录”冻结版本快照后通过现有 SSH/SFTP 写入全新版本目录，只把图片和 YOLO 标签写入
   它们所属的集合。全部文件成功且工作数据未变化才更新项目 `dataset_path`。修复了旧生成路径
   把每张图同时复制到 train/val 的问题。已运行训练引用的旧目录不覆盖；样本、标注、类别、划分或
   版本回退变化会使当前训练路径失效，需要重新生成。

数据所有权和失败分支：VLS/MySQL 保存项目、样本和不可变版本记录，对象存储保存实际媒体，
GPU 服务器仅接收生成后的训练文件；WVP/MQTT 不参与此流程。新增接口沿用登录校验，
查询和写入显式使用可信租户上下文并验证项目/样本/版本归属。迁移仅按所属项目补全历史样本、
标签、标注中缺失的租户字段，不改写已有非空归属。

文件损坏或不支持的格式返回具体导入结果；原始文件批量导入逐文件反馈。
ZIP 拒绝目录穿越、重复路径、超限解压和清单校验和不一致；导出先完整生成 ZIP，缺失对象会使请求失败，
不会把残缺包作为成功文件返回。逻辑删除保留底层媒体，以维持已有版本的恢复和导出能力。
并发版本操作通过项目行锁串行化，失败不会留下半个划分/回退结果。

`[边界]` 每个项目最多 10000 个样本；图片不超过 25 MB/4000 万像素，视频不超过 100 MB；
原始批次最多 100 文件，ZIP 最多 500 样本、压缩包 500 MB、解压后/单次导出 1 GB。
清晰度为启发式提示，纯色或低纹理图片可由人工复核为合格；视频结构检查不等同于逐帧解码。
未新增视频抽帧、视频时序标注或分类/分割 GPU 训练；当前训练生成仅支持物体检测矩形/圆形标注。
上传网关需允许相应请求体大小，应用限额可由 `VLS_UPLOAD_MAX_FILE_SIZE` / `VLS_UPLOAD_MAX_REQUEST_SIZE` 配置。

`[验证]` 2026-09-07：Java 8 编译、专用 MySQL 8 测试库中的真实迁移和业务事务、文件 ZIP 往返，
以及原标注进度共 22 项回归已验证；浏览器通过真实控制器/服务/MySQL 和隔离的本地测试对象存储
验证了项目/样本展示、划分、人工质量标记、版本对比、备份后回退和真实 ZIP 下载。
6 个样本及标注通过 HTTP ZIP 接口完整导入另一项目，改名、来源/标签筛选和原始文件去重也已验证。
前端构建通过；仓库缺少 ESLint 配置，
使用 `codex/data-management/eslint.cjs` 对新增页面/API 做定向静态检查。
正式环境迁移、真实 MinIO 访问和远端 GPU/SFTP 训练目录生成仍需要在更新后的运行环境验证，
不能用本地文件存储测试替代该结论。
...
| 2026-09-07 | CameraRTC 实时预览对齐原厂 `IE` 会话、DataChannel 和心跳协议，增加 ICE 短暂断连恢复、超时/失败限次重连、人工重试及分级 TURN TCP/TLS 回退 | `CameraRtcPlayer.vue`、`cameraRtcSession.js`、原厂页面对照及浏览器 CameraRTC 实播验证 |
