...
WebRtcStreamer.prototype.createPeerConnection = function() {
	...
	// Modern browsers emit ontrack. Register the deprecated onaddstream only as
	// a fallback: adapter.js may emit both events, and the legacy event can
	// overwrite the active MediaStream with an incomplete stream.
	if ("ontrack" in pc) {
		pc.ontrack = (evt) => {
			...
			this.videoElement.onloadedmetadata = () => {
				this.videoElement.play().catch((error) => console.warn("video play error:", error));
			};
...
WebRtcStreamer.prototype.onAddStream = function(event) {
	...
	if (this.videoElement.srcObject === event.stream) {
		return;
	}
