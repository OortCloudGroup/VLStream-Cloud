...
<template>
  <container-item-wrapper :widget="widget">
    ...
    >
      ...
      <el-row v-for="(subFormRowId, sfrIdx) in rowIdData" :key="subFormRowId" class="sub-form-row">
...
import { ElMessageBox, ElRow, ElButton, ElTooltip } from 'element-plus'

export default {
  name: 'SubFormItem',
  componentName: 'ContainerItem',
  components: { ElRow, ElButton, ElTooltip,
  ...
  methods: {
    ...
    deleteSubFormRow(formRowIndex) {
      ElMessageBox.confirm(this.i18nt('render.hint.deleteSubFormRow') + '?', this.i18nt('render.hint.prompt'), {
