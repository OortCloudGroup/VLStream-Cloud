...
<template>
  ...
  >
    ...
    >
      <template #prepend>
        <span> {{ field.options.moneyType }}</span>
      </template>
...
export default {
  ...
  methods: {
    ...
    fiexMonnyNum() {
      if (this.field.options.decimalLength) {
        this.fieldModel = Number(this.fieldModel).toFixed(this.field.options.decimalLength)
      }
    }
  }
}
