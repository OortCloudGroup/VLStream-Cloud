...
<template>
  <div class="option-items-pane">
    ...
    <div v-if="showImportDialogFlag" v-drag="['.drag-dialog.el-dialog', '.drag-dialog .el-dialog__header']" class="">
      <el-dialog
      ...
      >
        <el-form-item>
          <el-input v-model="optionLines" type="textarea" rows="10" />
        </el-form-item>
        <template #footer>
    ...
    <div v-if="showImportCascaderDialogFlag" v-drag="['.drag-dialog.el-dialog', '.drag-dialog .el-dialog__header']" class="">
      <el-dialog
        ...
        :destroy-on-close="true"
      >
        <code-editor ref="codeEditor" v-model="cascaderOptions" mode="json" :readonly="false" />
        <template #footer>
          <div class="dialog-footer">
...
import { ElButton, ElDialog, ElInput, ElRadio, ElRadioGroup, ElCheckbox, ElCheckboxGroup } from 'element-plus'
...
export default {
  ...
  methods: {
    changeAreaType() {
      if (this.areaType === 1) {
        this.optionModel.optionItems = province()
      }
      if (this.areaType === 2) {
        this.optionModel.optionItems = provinceCity()
      }
      if (this.areaType === 3) {
        this.optionModel.optionItems = provinceCityArea()
      }
    },
    importProviceCity() {
      this.$refs.codeEditor.setValue(JSON.stringify(provinceCity(), null, '  '))
    },
    importProviceCityArea() {
      this.$refs.codeEditor.setValue(JSON.stringify(provinceCityArea(), null, '  '))
    },
...
<style lang="scss" scoped>
  ...
  li.ghost{
    background: #fff;
    border: 2px dottedvar(--el-color-primary);
  }
