<template>
  <div class="data-page" v-loading="loading">
    ...
    <el-dialog v-model="sampleDialog" :title="reviewItems.length ? `人工抽检 ${reviewIndex + 1} / ${reviewItems.length}` : '样本详情与编辑'" width="960px" destroy-on-close :close-on-click-modal="false">
      ...
      <p class="hint">检查提示：{{ activeSample.qualityIssues || '无' }}<br />清晰度参考值：{{ activeSample.focusScore ?? '未检查' }}<br />检查时间：{{ activeSample.qualityCheckedAt || '未检查' }}</p>
...
const compareDialog = ref(false), compareFrom = ref(null), compareTo = ref(''), comparison = ref(null)
const loadComparison = async () => { comparison.value = await api.compareDataVersion(project.value.id, compareFrom.value.id, compareTo.value || undefined) }
const openCompare = row => run(async () => { compareFrom.value = row; compareTo.value = ''; comparison.value = null; await loadComparison(); compareDialog.value = true })
