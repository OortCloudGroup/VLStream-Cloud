<template>
  <section class="workflow-guide" aria-labelledby="data-workflow-title">
    ...
    <div v-if="expanded" id="data-workflow-content">
      <p class="workflow-description">从原始素材到可用数据集，在项目中完成数据导入、质量审核和标注，再划分训练集与验证集，通过版本记录保留每次调整。</p>
      <ol class="workflow-steps">
        <li v-for="(step, index) in steps" :key="step.title" class="workflow-step">
          <div class="step-visual">
            ...
            <el-icon v-if="index < steps.length - 1" class="step-arrow"><ArrowRight /></el-icon>
          </div>
          <h4>{{ step.title }}</h4>
          <p>{{ step.description }}</p>
          <button type="button" class="step-result" :disabled="disabled" @click="emit('navigate', index)">{{ step.result }} <el-icon><ArrowRight /></el-icon></button>
        </li>
      </ol>
      <div class="workflow-tip"><el-icon><InfoFilled /></el-icon><span>{{ hasProject ? '在「样本管理」中导入、筛选和审核素材，点击「进入标注」编辑标注，再到「数据集划分与版本」保存训练数据。' : '开始使用：新建项目，或点击下方项目的「管理数据」，按流程完成数据准备。' }}</span></div>
    </div>
  </section>
</template>
...
<script setup>
import { ref } from 'vue'
import { ArrowDown, ArrowRight, ArrowUp, CircleCheck, EditPen, Files, FolderOpened, InfoFilled, UploadFilled } from '@element-plus/icons-vue'

defineProps({ hasProject: { type: Boolean, default: false }, disabled: { type: Boolean, default: false } })
const emit = defineEmits(['navigate'])
const expanded = ref(true)
const steps = [
  { title: '创建项目', icon: FolderOpened, description: '填写项目名称、编号和说明，选择标注类型，明确样本用途与标注规则。', result: '统一管理数据与标注配置' },
  { title: '导入数据', icon: UploadFilled, description: '批量导入图片、视频或 ZIP，记录数据来源，检查文件格式和完整性。', result: '形成可检索的原始样本' },
  { title: '质量筛选', icon: CircleCheck, description: '检查图片清晰度和标注情况，结合人工抽检，标记低质量或剔除异常样本。', result: '筛选可用素材' },
  { title: '数据标注', icon: EditPen, description: '进入标注页面维护类别，为图片添加标注；视频可播放抽检，保留为原始素材。', result: '形成带类别与标注的图片样本' },
  { title: '数据集与版本', icon: Files, description: '按比例或手动划分训练集和验证集，查看类别分布，保存、对比和回退版本。', result: '导出数据或生成训练目录' }
]
</script>
...
.workflow-steps { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 28px; list-style: none; margin: 24px 0 20px; padding: 0; }
