<template>
  <section class="import-history">
    ...
    <el-table :data="jobs" max-height="300" empty-text="暂无导入任务">
      ...
      <el-table-column :label="$tp('任务')" width="100"><template #default="{ row }">{{ row.importType === 'video-frames' ? '视频切图' : '数据导入' }}</template></el-table-column>
      <el-table-column prop="sourceName" :label="$tp('来源')" width="100" show-overflow-tooltip />
      <el-table-column :label="$tp('状态')" width="120"><template #default="{ row }"><el-tag :type="stateColor(row.jobState)">{{ states[row.jobState] || row.jobState }}</el-tag></template></el-table-column>
      <el-table-column :label="$tp('进度与结果')" min-width="190"><template #default="{ row }"><span v-if="row.jobState === 'PROCESSING' && row.progressMessage">{{ row.progressMessage }}</span><span v-else>成功 {{ row.importedFiles || 0 }} / 跳过 {{ row.skippedFiles || 0 }} / 失败 {{ row.failedFiles || 0 }}</span></template></el-table-column>
      <el-table-column :label="$tp('操作')" width="145"><template #default="{ row }"><el-button link @click="details = row">详情</el-button><el-button v-if="['FAILED', 'PARTIAL'].includes(row.jobState)" link type="primary" @click="retry(row)">重试</el-button><el-button v-if="['UPLOADING', 'QUEUED'].includes(row.jobState)" link type="danger" @click="cancel(row)">取消</el-button></template></el-table-column>
    </el-table>
