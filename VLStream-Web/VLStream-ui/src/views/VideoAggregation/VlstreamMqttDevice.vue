        ...
        <template v-if="detail?.latestTask">
          ...
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="状态">
              <span>{{ taskStatusLabel(detail.latestTask.deployStatus) }}</span>
              <el-button v-if="isActiveTaskStatus(detail.latestTask.deployStatus)" link type="danger"
...
import {
  cancelMqttDeviceFirmwareTask, closeMqttDevicePreview, createMqttDevicePreview, deployMqttDeviceFirmware, getMqttDeviceDetail,
...
async function cancelLatestTask() {
  const task = detail.value?.latestTask
  if (!detailDeviceId.value || !task?.requestId || !isActiveTaskStatus(task.deployStatus)) return
  try {
    await ElMessageBox.confirm(
      '终止只会解除平台任务锁，不会向设备发送取消指令。请先确认设备已停止升级，再重新下发固件。',
      '终止 OTA 任务',
      { type: 'warning', confirmButtonText: '确认终止', cancelButtonText: '取消' }
    )
    cancellingTask.value = true
    await cancelMqttDeviceFirmwareTask(detailDeviceId.value, task.requestId)
    ElMessage.success('OTA 任务已终止，可以重新下发固件')
    await loadDetail()
  } catch (error) {
    if (error !== 'cancel' && error !== 'close') {
      ElMessage.error(error?.response?.data?.msg || error?.message || '终止 OTA 任务失败')
    }
