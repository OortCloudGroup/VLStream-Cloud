...
// getUserInfo APIdata
console.log('🔧 测试getUserInfo API数据格式修复...')

async function testGetUserInfoFix() {
  console.log('🧪 开始测试getUserInfo API数据格式修复...')
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (result.code === 200 && result.data) {
        ...
        console.log('📋 修复后的用户信息解析:')
        console.log('- 用户名称:', userData.user_name)
        console.log('- 用户ID:', userData.user_id)
        console.log('- 登录ID:', userData.login_id || userData.user_name)
        ...
        console.log('✅ 数据格式修复成功，应该能正确显示用户信息')

        // need to field
        const requiredFields = ['user_name', 'user_id', 'tenant_id']
        ...
        console.log('❌ API返回数据格式仍然不正确:', result)
      }
    ...
    console.error('❌ 测试getUserInfo API修复失败:', error)
  }
}
