...
public class UserServiceImpl extends BaseServiceImpl<UserMapper, User> implements IUserService {
	...
	public boolean submit(User user) {
		...
		if (cnt > 0) {
			throw new ServiceException("Current user already exists!");
		}
	...
	public boolean updateUserInfo(User user) {
		// User modifying own info forcibly specifies the ID of the current requesting account
		user.setId(SecureUtil.getUserId());
		User currentUser = getById(user.getId());
		if (currentUser == null) {
			throw new ServiceException("User does not exist!");
		}
		// User modifying own info forcibly ignores fields like roles, departments, accounts, etc.
		user.setRoleId(null);
		user.setDeptId(null);
	...
	public boolean updatePassword(Long userId, String oldPassword, String newPassword, String newPassword1) {
		...
		if (!user.getPassword().equals(DigestUtil.encrypt(oldPassword))) {
			throw new ServiceException("Incorrect original password!");
		}
	...
	public boolean registerGuest(User user, Long oauthId) {
		...
		if (tenant == null || tenant.getId() == null) {
			throw new ServiceException("Incorrect tenant info!");
		}
		UserOauth userOauth = userOauthService.getById(oauthId);
		if (userOauth == null || userOauth.getId() == null) {
			throw new ServiceException("Third-party login info error!");
		}
