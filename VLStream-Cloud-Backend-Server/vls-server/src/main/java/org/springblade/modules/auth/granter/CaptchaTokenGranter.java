...
/**
 * Verification code TokenGranter
 *
 * @author Chill
 */
@Slf4j
...
public class CaptchaTokenGranter implements ITokenGranter {
	...
	public UserInfo grant(TokenParameter tokenParameter) {
		...
		// Determine if account and IP are locked
		...
		if (Func.isNoneBlank(account, password)) {
			...
			// Call the corresponding interface to return data based on different user types, which can be extended by users
		...
		if (userInfo == null || userInfo.getUser() == null) {
			...
			log.error("User login failed, account: {}, IP: {}", account, WebUtil.getIP());
