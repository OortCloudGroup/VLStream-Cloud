...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements IMenuService {
	...
	public List<MenuVO> routes(String roleId, Long topMenuId) {
		...
		// If no top menu ID is specified, return all menus corresponding to role
		if (Func.isEmpty(topMenuId)) {
			// Get menus configured for role
			List<Menu> roleIdMenus = baseMapper.roleMenuByRoleId(Func.toLongList(roleId));
			// Reverse recursively get all parent levels of the role menu
			List<Menu> routes = new LinkedList<>(roleIdMenus);
			roleIdMenus.forEach(roleMenu -> recursion(allMenus, routes, roleMenu));
			roleMenus = routes;
		}
		// If Top menu ID is specified, return the intersection of RoleMenu and Top menu
		else {
			// Get menus configured for role
			List<Menu> roleIdMenus = baseMapper.roleMenuByRoleId(Func.toLongList(roleId));
			// Reverse recursively get all parent levels of the role menu
			List<Menu> routes = new LinkedList<>(roleIdMenus);
			roleIdMenus.forEach(roleMenu -> recursion(allMenus, routes, roleMenu));
			// Get menus configured for top menu
			List<Menu> topIdMenus = baseMapper.roleMenuByTopMenuId(topMenuId);
			// Filter permission menus matching roles (intersection)
