...
/**
 * Single-device semantic segmentation session: periodically pull images (URL/local path) and perform semantic segmentation, outputting the segmented image and creating an event when a result is generated.
 */
@Slf4j
public class DeviceSemSegDetectionSession implements DeviceDetectionSession {
        ...
        Thread thread = new Thread(() -> {
            while (!stopRequested.get()) {
                try {
                    ...
                    log.warn("Device {} semantic segmentation processing failed", deviceInfo.getDeviceName(), exception);
                }
    ...
    private void runDetectionOnce() {
        ...
        try {
            ...
            if (result == null || !result.isSuccess() || result.getData() == null) {
                ...
                log.warn("Device {} semantic segmentation failed: {}", deviceInfo.getDeviceName(), message);
                return;
            }
    ...
    private Image loadImage() {
        ...
        log.warn("Device {} has no available image address configured, stopping semantic segmentation", deviceInfo.getDeviceName());
        stopRequested.set(true);
        return null;
    }
    ...
    private Image loadImageFromPath(String imagePath) {
        ...
        try {
            ...
            log.warn("Device {} failed to load image: path={}", deviceInfo.getDeviceName(), imagePath, exception);
            return null;
        }
    }
    ...
    private void createEvent(DeviceInfo deviceInfo,
                             ...
                             CategoryMask mask,
                             String snapshotPath) {
        String eventDesc = "Device" + deviceInfo.getDeviceName() + "Complete semantic segmentation";
