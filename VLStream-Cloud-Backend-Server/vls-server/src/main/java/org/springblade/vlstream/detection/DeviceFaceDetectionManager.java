...
/**
 * Device face detection task manager: Scans device configurations periodically and maintains face detection sessions.
 */
@Slf4j
@Component
...
public class DeviceFaceDetectionManager extends AbstractDeviceDetectionManager<DeviceFaceDetectionSession> {
    ...
    protected DesiredDeviceConfig buildDesiredConfig(DeviceInfo deviceInfo) {
        ...
        if (StringUtils.isBlank(streamUrl)) {
            log.warn("Device {} has no stream address configured, skipping face detection", deviceInfo.getDeviceName());
            return null;
        }
    ...
    protected String getMissingConfigReason() {
        return "Device face detection algorithm not configured or configuration incomplete";
    }

    @Override
    protected String getConfigChangedReason() {
        return "Device face detection configuration changed";
    }

    @Override
    protected String getRefreshErrorMessage() {
        return "Failed to refresh device face detection task";
    }

    @Override
    protected String getStopErrorMessage() {
        return "Failed to stop device face detection: deviceId={}, reason={}";
    }
