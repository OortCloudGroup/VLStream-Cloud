...
/**
 * Single-device face detection session: load the model, perform face detection on the video stream, take a screenshot, upload it, and create an event when a result is generated; support abnormal restart and resource cleanup.
 */
@Slf4j
public class DeviceFaceDetectionSession implements DeviceDetectionSession {
        ...
        return new StreamDetector.Builder()
            ...
            .listener(new StreamDetectionListener() {
                ...
                public void onObjectDetected(List<DetectionInfo> detectionInfoList, Image image) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            log.info("Face detection snapshot saved: deviceId={}, deviceName={}, time={}, path={}",
                        ...
                        log.warn("Device {} face detection callback processing failed", deviceInfo.getDeviceName(), detectException);
                ...
                public void onStreamEnded() {
                    log.info("Face detection video stream ended: deviceId={}, deviceName={}, algorithmId={}",
    ...
    private void scheduleDetectorRestart(String reason) {
        ...
        Thread restartThread = new Thread(() -> {
            try {
                ...
                log.info("Restart triggered for face detection: deviceId={}, deviceName={}, algorithmId={}, reason={}",
                    deviceInfo.getId(), deviceInfo.getDeviceName(), algorithmId, reason);
                stopDetector();
                Thread.sleep(START_RETRY_DELAY_MILLIS);
                if (stopRequested.get()) {
    ...
    private void stopDetector() {
        ...
        try {
            ...
            log.warn("Failed to stop face detector", stopException);
        }
        try {
            currentDetector.close();
        } catch (Exception closeException) {
            log.warn("Failed to close face detector", closeException);
        }
    }
    ...
    private void createEvent(DeviceInfo deviceInfo,
                             ...
                             String snapshotPath) {
        String eventDesc = "Device" + deviceInfo.getDeviceName() + "Detected" + detectionInfos.size() + "faces";
