...
public class VlsMqttPublishService {
	...
	public boolean publish(String topic, Object payload) {
		if (StringUtils.isBlank(topic) || !StringUtils.startsWith(topic, "oortcloud/")) {
			log.error("Failed to send MQTT message, topic only allows oortcloud/#: topic={}", topic);
			return false;
		}
		...
		try {
			...
			log.error("MQTT message sending exception: topic={}", topic, publishException);
			return false;
		} finally {
