...
/**
 * Single-device image classification detection session: load the model, classify the video stream, take a screenshot, upload it, and create an event when a result is generated; support abnormal restart and resource cleanup.
 */
@Slf4j
public class DeviceClassifyDetectionSession implements DeviceDetectionSession {
            ...
            .listener(new ClsStreamDetectionListener() {
                ...
                public void onObjectDetected(List<DetectionInfo> detectionInfoList, Image image) {
                    ...
                    try {
                        ...
                        log.warn("Device {} image classification callback processing failed", deviceInfo.getDeviceName(), detectException);
                ...
                public void onStreamEnded() {
                    log.info("Image classification video stream ended: deviceId={}, deviceName={}, algorithmId={}",
    ...
    private void scheduleDetectorRestart(String reason) {
        ...
        Thread restartThread = new Thread(() -> {
            try {
                ...
                log.info("Restart triggered for image classification: deviceId={}, deviceName={}, algorithmId={}, reason={}",
                    deviceInfo.getId(), deviceInfo.getDeviceName(), algorithmId, reason);
                stopDetector();
                Thread.sleep(START_RETRY_DELAY_MILLIS);
                if (stopRequested.get()) {
    ...
    private void stopDetector() {
        ...
        try {
            ...
            log.warn("Failed to stop image classifier", stopException);
        }
        try {
    ...
    private void createEvent(DeviceInfo deviceInfo,
                             ...
                             String snapshotPath) {
        String eventDesc = "Device" + deviceInfo.getDeviceName() + "Classified to" + detectionInfos.size() + "results";
