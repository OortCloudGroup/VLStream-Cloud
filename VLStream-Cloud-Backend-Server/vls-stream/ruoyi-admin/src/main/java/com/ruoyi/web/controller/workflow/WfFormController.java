...
public class WfFormController extends BaseController {
    ...
    @PostMapping
    public R<WfForm> add(@RequestBody WfFormBo bo) {
        return R.ok(formService.insertForm(bo));
    }
    ...
    public R<Void> remove(@NotEmpty(message = "主键不能为空") @PathVariable Long[] formIds) {
        Boolean success = formService.deleteWithValidByIds(Arrays.asList(formIds));
        if (!success) {
            return R.fail("该表单正在被引用，无法删除");
        }
